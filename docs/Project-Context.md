...
**═══ SorTrack is PAUSED as of 2026-07-10. Read "Session 2026-07-10" below before touching this project. New active project (AI BOQ takeoff) is being scoped separately — see that session entry. ═══**

**Why paused**: after the Pojjaman-integration pivot (2026-07-09, below), a follow-up call with the aunt concluded the mobile-capture idea doesn't actually save a step — site leaders already have to key data at their own site to track it there, so routing through a separate app and handing output back to the office adds a hop rather than removing one. Decision: **hold the app idea, integrate Pojjaman directly, train site leaders on Pojjaman itself** (their real per-user footprint is 1-2 repetitive screens, not the whole ERP). Re-evaluate only if real-world Pojjaman adoption struggles. **Do not resume feature work on SorTrack without a new explicit instruction from the user** — Phase A stands as a finished, demoed portfolio piece, not an active build.

**═══ STRATEGIC PIVOT #2 (2026-07-09, historical — superseded by the pause above, kept for record): SorTrack was reframed as a mobile field-capture front-end for Pojjaman, NOT a standalone store system. ═══**

The family company already runs **Pojjaman (พจมาน)** — a mature Thai construction ERP that does BOQ, PR→PO, cost centers, a 5-level item catalog, multi-store inventory (withdraw/return/transfer), and full equipment lifecycle (depreciation, repair, write-off). SorTrack cannot and must not out-build it. The meeting reframed SorTrack's whole reason to exist: **the mobile-friendly, low-tech-friendly capture layer for site leaders and storage keepers**, reading PO/item data from Pojjaman (read-only SQL/API over VPN, granted in the meeting) and handing the office a copy-pastable output to log. Pojjaman = office system of record. SorTrack = field front door that feeds it. Details, gaps, and the backlog as of that pivot are in the 2026-07-09 session entry below — largely moot now given the pause, but kept as a record of the discovery and the integration path (test DB, read-only SQL/API, VPN) in case it's ever revisited.
...
### Session 2026-07-10: SorTrack paused; AI BOQ takeoff proposed as next project
- **Goal**: Follow-up call with the aunt after the 2026-07-09 demo/discovery; two more live demos run with fresh test data; decide SorTrack's fate; capture a new project idea she raised.

- **Two more demo walkthroughs run live** (not scripted via SQL — typed in through the UI, for practice/demo purposes):
  - PO `SC10-6907-0012` (ไทยวัสดุก่อสร้าง, โครงการ BR69011): ไม้สัก 10 @ 450 + ปูนซีเมนต์ 30 @ 150 → received with ไม้สัก 1 rejected (damaged, returned) → withdrew ปูนซีเมนต์ 10.
  - PO `SC10-6907-0013` (เอส.ซี. สตีล แอนด์ ไพพ์, โครงการ BR69022, at the second unit หน่วยเชียงใหม่): เหล็กเส้น 20 @ 700 + ท่อ PVC 15 @ 100 → received with 3 ท่อ PVC rejected (wrong item, returned) → withdrew เหล็กเส้น 8. Demonstrated stock accumulating on top of the multi-location seed data, not just fresh zero-state.

- **THE DECISION — SorTrack is PAUSED, not killed.** Reasoning from the aunt, and it holds up: site leaders already need to key data at their own site to track it there regardless. Routing through a separate mobile app and then handing a copy-pastable summary back to the office for re-entry **adds a step instead of removing one** — it doesn't solve the actual adoption problem (workers not entering data at all). Her call: **integrate Pojjaman directly, train site leaders on Pojjaman itself.** Her reasoning for why training is viable despite Pojjaman's complexity: a given site leader's real day-to-day footprint is only 1-2 repetitive screens (receive, withdraw) with repetitive item codes — not the whole ERP surface. Once trained on that narrow slice, adoption should hold. **Explicit instruction: hold on the app, see how real-world Pojjaman adoption goes first**, revisit only if it struggles.
  - This does not erase the work. Phase A is a complete, working, demoed full-stack app (Next.js/FastAPI/Supabase, redesigned UI, live end-to-end flow) built and presented to real client management and an ERP vendor in a live joint meeting — a legitimate finished portfolio piece regardless of production status. Resume-phrasing guidance: describe what was built and demoed, not "stopped" — e.g. "Built and demoed a full-stack material-tracking app to company management and their ERP vendor; findings shaped the company's integration decision." True, complete, and reads as scoping judgment rather than failure.
  - **Action for future sessions**: do not pick SorTrack feature work back up without an explicit new instruction from the user. Treat it as shelved/complete-as-is.

- **NEW PROJECT PROPOSED (not started, exploration only): AI-assisted BOQ takeoff from CAD/แบบ.** Aunt described the company's real, current, expensive problem:
  - **The problem (ตกหล่น)**: when estimating a project from architectural drawings (แบบ), engineers manually read CAD/plans and calculate a BOQ (bill of quantities). Materials get missed — not because they're stated and skipped, but because the drawing *implies* a need (e.g., a corner/detail that requires extra material) without stating it outright, and it takes engineering judgment to catch it. Missed items mean the company either eats the cost out of pocket later or it was never quoted to the client. Craftsmen/estimators also habitually understate material needs early on to avoid overwhelming the client, then reality costs more. Manual takeoff for a large project (~400M budget scale mentioned) takes a person 1-2 months.
  - **Her ask**: can AI read a CAD file (2D, possibly future 3D) and draft a BOQ in 1-2 days instead of 1-2 months, for a human engineer to check/correct rather than build from scratch? Over time, "train" the system to recognize company-specific patterns/techniques (adapt from corrections).
  - **User's stated worries, addressed honestly in-session (see chat, not fully re-derived here)**: does this need a dedicated ML/PhD team (no — this is a pipeline + prompting problem, not a model-training problem, given current LLM capability); can AI actually read CAD (DWG is binary/unreadable by LLMs directly, DXF is text-based and parseable via libraries like `ezdxf`; LLM vision on drawings is decent for classification/understanding but not reliable for precise measurement — so the honest architecture is code parses geometry, AI reasons over the parsed structure, not "point an LLM at a CAD file and get numbers back"); API cost is low (tens of baht per document, not a budget concern); this is NOT the same shape of problem as the earlier abandoned fitness-app body-analysis feature (that one died on privacy + needing near-medical accuracy with no human backstop; this one has no personal data and the aunt herself already frames it as "draft output, human checks it," which matches what's honestly deployable today).
  - **Recommended honest scope for an MVP** (from chat, not yet actioned): pick one drawing type (e.g. structural floor plans) → DXF in → categorized/quantified element list where measurable → explicit flags for anything uncertain rather than silently guessing → spreadsheet out. Prove it against 2-3 real past projects before expanding. Explicitly do NOT promise the hardest part (inferring unstated-but-implied materials from engineering judgment) as an MVP feature — that's the actual hard research problem, not a v1 scope item.
  - Aunt separately also raised wanting to improve the company's public visibility/brand (put the company name out there) — unconnected to the AI idea, noted for later, not scoped.
  - **Status: idea only, not started.** No repo, no spec doc yet. If picked up, treat as a new project (new spec doc, likely new repo or a clearly separate module) rather than folding into SorTrack.

- **Context for whoever resumes**: this session followed an exhausting ~16-hour stretch (1pm–5am) across two major strategic pivots in two days (Pojjaman-integration reframe on 07-09, then the pause on 07-10). User was explicitly emotionally rattled at points during 07-09 (recovered well) and flagged time pressure re: portfolio/recruiting season. If resuming either thread, don't relitigate the pause decision or re-open SorTrack scope without the user raising it first — both were deliberate, reasoned conclusions, not things to second-guess by default.
- **Next**: if the AI BOQ project is picked up, start with a proper scoping session (new spec doc) — do not start writing code from this summary alone, the architecture (DXF parsing pipeline vs. vision-only vs. hybrid) needs to be decided deliberately, and real sample CAD/แบบ files from the company should ground it, same lesson as SorTrack's v2 pivot (ground design in the real subject, not assumptions).
